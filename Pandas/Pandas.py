...
# СОЗДАНИЕ
# # data = pd.Series([0, 0.1, 0.2, 0.3]) передача питоновского списка
...
# # print(data.index)  # RangeIndex(start=0, stop=4, step=1) - стандартный индекс в пандасе
...
# data[:-2] - вывод всех кроме двух последник
# data['a':'c'] - "c" уже включается в вывод
# # ИНДЕКСАТОРЫ loc, iloc - поиск осуществлять тольки ими ([], .ix[] - нельзя)
...
# table[2:4] - вывод второго и третьего столбца
# ВЫВОД СТРОКИ
# table.loc['aa'] - вывод строки по метке индекса
# table.loc[['aa', 'bb']] - вывод несколько строк по списку меток индекса
# table.iloc[1] - вывод второй строки по позиции
# table.iloc[[1, 2]] - вывод несколько строк по позиции
